...
for (i in 1:nvars) {
  ...
  # 1. Time Series Plot using autoplot()
  p1 <- autoplot(ts_energy_data2[, i]) +
    geom_hline(yintercept = mean(ts_energy_data2[, i], na.rm = TRUE), 
               color = "red", linetype = "dashed") +
    labs(title = paste("Time Series of", colnames(ts_energy_data2)[i]),
         x = "Time", y = colnames(ts_energy_data2)[i]) +
    theme_minimal()

  # 2. ACF Plot converted to ggplot
  p2 <- as.ggplot(function() Acf(ts_energy_data2[, i], lag.max = 40, 
                                        main = paste("ACF of", colnames(ts_energy_data2)[i])))

  # 3. PACF Plot converted to ggplot
  p3 <- as.ggplot(function() Pacf(ts_energy_data2[, i], lag.max = 40, 
                                          main = paste("PACF of", colnames(ts_energy_data2)[i])))

  # Arrange the three plots side by side using patchwork
...
```{r}
# Time Series Plot
p1 <- autoplot(ts_energy_data2[, "Total Renewable Energy Production"]) +
  geom_hline(yintercept = mean(ts_energy_data2[, "Total Renewable Energy Production"],
                               na.rm = TRUE), 
             color = "red", 
             linetype = "dashed") +
  labs(title = "Time Series of Renewable Energy Production",
       x = "Time", 
       y = "Renewable Energy Production") +
  theme_minimal()

p2<-Acf(ts_energy_data2[, "Total Renewable Energy Production"],
        lag.max=40,
        main=paste(" ACF: Total Renewable Energy Production",sep="")) 

p3 <- Pacf(ts_energy_data2[, "Total Renewable Energy Production"], 
           lag.max = 40,
        main=paste(" ACF: Total Renewable Energy Production",sep="")) 

RE_plot <- p1 + p2 + p3 + plot_layout(ncol = 3)
print(RE_plot)
```
